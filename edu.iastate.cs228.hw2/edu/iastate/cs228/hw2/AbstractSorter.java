...
public abstract class AbstractSorter {
	...
	protected AbstractSorter(Point[] pts) throws IllegalArgumentException {
		if (pts.length == 0 || pts == null) {
			...
			for (int i = 0; i < pts.length; i++) {
				// note: this is technically copying the pointers for the Point objects, not the
				// object itself. Shallow copy.
				points[i] = pts[i];
			}
		}
	}
	...
	public void setComparator(int order) throws IllegalArgumentException {
		if (order < 0 || order > 1) {
			// order is less than 0 or greater than one, throwing exception
			throw new IllegalArgumentException();
		} else if (order == 0) {
	...
	public void getPoints(Point[] pts) {
		// copies values from points[] into pts[].
