...
class IteratorTests {
	...
	private <T extends Comparable<? super T>> void hasNextTester(int nodeSize, Object[] data) {
		...
		for (int i = 0; i < dataArray.length; i++) {
			assertTrue(list.listIterator(i - 1).hasNext(), "Expected to iterator to say it has next element.");
		}
	...
	private <T extends Comparable<? super T>> void nextTester(int nodeSize, Object[] data) {
		...
		for (int i = 0; i < dataArray.length; i++) {
			assertEquals(dataArray[i], list.listIterator(i - 1).next(),
	...
	@Test
	void SetTest() {
		String expected = "[(2048.34, -42.67, 11.53, 54.6, 75.32, -26.77, 0.46, 39.81, 17.95, -54.29, -5.14, 62.71), (545.567, | -72.09, -14.62, 5.41, 20.19, -95.76, 81.63, 0.002, -, -, -, -)]";
	...
	void NextIndexTest() {
		...
		assertEquals(8, iter.nextIndex(), "nextIndex() gave incorrect result.");
		iter.next();
		assertEquals(9, iter.nextIndex(), "nextIndex() gave incorrect result after incrementing with next().");
	}
	...
	void PreviousIndexTest() {
		...
		iter.previous();
		assertEquals(5, iter.previousIndex(), "previousIndex() gave incorrect result after decrementing with previous().");
	}
